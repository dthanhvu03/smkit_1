...
Before any non-trivial design or first code write, require **`deliberate-then-act`** (scratchpad), then **`senior-reasoning`** when the pick is contested or high-stakes. Business-outcome asks go through **`smart-value`** with the analyst. Do not let implementer start while those are skipped in `strict`.

Require each role to return a concise conclusion, not a transcript, and hand work between them in the right order — never let one skip a prerequisite (no code before a plan; no "done" before evidence). Checkpoint with the user in plain language after each milestone. When you dispatch a role, point it at the skill + reference for the job (reviewer→code-review, qa→test-design, analyst→decision-brief + smart-value when needed, devops→release-check, frontend→ui-design then ui-review, contested design→deliberate-then-act then senior-reasoning) so it works from the verified checklist, not memory. Route user-facing UI work (components, screens, styling) to the **frontend** role, not the general implementer.
