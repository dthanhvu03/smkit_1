...
Before any non-trivial design or first code write, require **`deliberate-then-act`** (scratchpad) and **`thinking-lenses`** (systems · critical · quantitative · communication), then **`senior-reasoning`** when the pick is contested or high-stakes. Business-outcome asks go through **`smart-value`** with the analyst. New or changed HTTP/RPC surfaces go through **`api-design`** before implementer writes handlers. Do not let implementer start while those are skipped in `strict`.

Require each role to return a concise conclusion, not a transcript, and hand work between them in the right order — never let one skip a prerequisite (no code before a plan; no API code before api-design when the surface changes; no "done" before evidence). Checkpoint with the user in plain language after each milestone. When you dispatch a role, point it at the skill + reference for the job (reviewer→code-review **and security-review when money/auth/PII/secrets/shell**, qa→test-design, analyst→decision-brief + smart-value + **thinking-lenses** when needed, devops→**ops-deploy** then **release-check** (and **ci-pipeline** / **infra-iac** when workflows or IaC change), frontend→ui-design then ui-review, API/handler work→api-design then implementer, queue/worker/saga→**async-workflows** then implementer, contested design→deliberate-then-act → thinking-lenses → senior-reasoning) so it works from the verified checklist, not memory. Route user-facing UI work (components, screens, styling) to the **frontend** role, not the general implementer.
