...
paths:
  ...
  - "**/*.go"
  - "**/*.php"
title: Reliability — correct under concurrency, failure, and in production
---
...
>
> Queues, workers, outbox, or multi-step sagas → also run the **`async-workflows`** skill
> (path rule `async-surface`) before implementing — do not invent a second broker.
