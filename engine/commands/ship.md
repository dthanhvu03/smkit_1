...
4. **Design** — if the approach isn't obvious, **diverge first** (the **brainstorm** skill) to surface several candidate approaches before narrowing. Then for anything non-trivial or contested, **reason it through like a senior** (the **senior-reasoning** skill): challenge the premise, weigh 2+ options with real numbers, name the non-obvious risk, steelman the leading choice — depth, not a checklist pass. Then the planner breaks it into the smallest useful slices with a definition of done; architect fixes structure and records decisions. For a new feature with a lifecycle or a high-stakes rule (money · booking · inventory), run the **domain-model** skill — name the entities, the states and legal transitions, and the invariants — so illegal states are designed out, not patched later. For a change touching shared data or behavior, run the **impact-map** skill — map every caller, job, event, and test it touches before editing — and record both in the task. If a choice is contested, run `/roundtable` (bounded — converge or escalate, never loop forever).
5. **Build** — start on a `feature/*` branch (**git-workflow** skill — never on `main`/`dev`); implementer writes each slice following existing patterns, committing small, atomic Conventional Commits as it goes. For high-stakes paths (money · booking · inventory · external calls) the **reliability** rule applies — idempotent retries, guard shared state at the DB, timeouts, and observable failures. Pull in specialists as needed: db-admin (schema/migrations), debugger (failures), git-manager (branches/commits/PR), docs-manager (docs ↔ code).
