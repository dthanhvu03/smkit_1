---
id: challenge
title: Challenge a change before building it
description: Run the pre-build critique — stress-test the planned change through fixed lenses and record a go/adjust/stop verdict before any code is written.
...
2. Run the **pre-build-critique** skill: challenge it through every lens — correctness, security & data, consistency, simplicity/necessity, reversibility (one-way vs two-way door), and a **pre-mortem** (imagine it failed months later — the single most likely reason). If no scratchpad exists yet this turn, run **`deliberate-then-act`** first (include **`thinking-lenses`**: systems · critical · quantitative · communication). Reason at **senior depth** (the **senior-reasoning** skill): each finding carries a *why* + a *consequence*, not a one-line "looks fine"; steelman the change before you clear it.
