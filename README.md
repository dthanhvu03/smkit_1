...
rules/profiles in place. Installing into an **existing** project is safe. Your `.claude/settings.json` is **merged**, not
replaced — the kit adds its hooks and deny rules while keeping your own hooks, permissions, `env`,
and MCP servers. Any other file the kit generates that you already have (`CLAUDE.md`, `AGENTS.md`,
…) is saved to `<file>.bak` before it's overwritten, and anything the kit doesn't generate is
never touched.
...
/challenge   stress-test a change through lenses + a pre-mortem before coding (a hook enforces this in standard/strict)
/task        open a work record — scope, acceptance, impact map, plan — before building; traceable & resumable
/resume      new chat mid-feature — re-read .kit/ SoT + active task + handoff, restate, then continue (no drift)
...
/handoff     package a delivery for the owner — what was built, proof, impact, what to approve
/review      static review of the diff · /checkup consistency audit · /decide record a decision (ADR)
/postmortem  after an incident — blameless root cause + the fix that stops the whole class
```
