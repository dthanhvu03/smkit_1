...
switch (cmd) {
  ...
  case "eval": r = run("eval.mjs", rest); break;
  case "build":
  case "check":
  case "doctor": r = run("kitgen.mjs", [cmd, ...rest]); break;
  default:
    console.log("Usage: smkit <init|update|uninstall|build|check|doctor|eval> [options]\n" +
      ...
      "  eval       score the hard-tier guardrails (do they actually enforce?) — --json for CI");
