...
4. **Produce a decision brief** using the `decision-brief` skill (and its `references/decision-guide.md` — Jobs-to-be-Done, MVP/smallest slice, one-way vs two-way door) and its required output — including the **`thinking-lenses`** block (systems · critical · quantitative · communication) — problem, users, 2–3 options with trade-offs, a rough (labelled) effort/cost/risk estimate, the smallest slice that teaches us the most, the questions only the founder can answer, and a recommendation. Fold the `smart-value` recommendation into the brief when that skill ran.
5. **Checkpoint before build.** Present it in plain language (Communication lens) and get a decision. Never let planning or code start on a vague request that has not been through this.
